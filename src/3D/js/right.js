...
function init() {
	...
	loader.load( '/3D/files/Tree.obj',
		function ( object ) {
			const positions = combineBuffer( object, 'position' );
			createMesh( positions, scene, 200, 400, -300, 1200, 0x0f0c4f );
			createMesh( positions, scene, 200, 400, -380, 1000, 0xdd2a57 );
		} 
	);
...
function createMesh( positions, scene, scale, x, y, z, color ) {
	...
	meshes.push({
		...
		direction: 0, 
		speed: 0.07, 
		delay: Math.floor( 200 + 100 * Math.random() ),
		start: Math.floor( 100 + 100 * Math.random() ),
	});
}
		...
		for ( let i = 0; i < count; i ++ ) {
			...
			if ( data.direction < 0 ) {
				if ( py > 0 ) {
					positions.setXYZ(
						i,
						px + 1.5 * ( 0.80 - Math.random() ) * data.speed * delta,
						py + 3.0 * ( 0.55 - Math.random() ) * data.speed * delta,
						pz + 1.5 * ( 0.80 - Math.random() ) * data.speed * delta
					);
				} else {
					data.verticesDown += 1;
				}
