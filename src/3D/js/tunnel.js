import * as THREE from './libs/three.moduleOld.js';
...
Tunnel.prototype.init = function() {
  ...
  this.mouse = {
    ...
    target: new THREE.Vector2(ww * 0.1, wh * 0.1)
  };
  ...
  this.camera = new THREE.PerspectiveCamera(20, ww / wh, 0.01, 10);
...
Tunnel.prototype.createMesh = function() {
  ...
  geometry.vertices = this.curve.getPoints(240);
  ...
  this.tubeGeometry = new THREE.TubeGeometry(this.curve, 480, 0.2, 3, false);
  ...
  for (let i = 0; i < this.tubeGeometry.faces.length; i++) {
    ...
    p = this.tubeGeometry.vertices[f.a];
      color = new THREE.Color(
        "hsl(" +
            (Math.floor(
              Math.abs(noise.simplex3(p.x * 2, p.y * 4, p.z * 2)) * 140 * 100
            ) *
              0.01 +
              180) +
          ",70%,60%)"
...
Tunnel.prototype.updateCurve = function(delta) {
  ...
  for (let i = 0; i < this.tubeGeometry.faces.length; i++) {
    f = this.tubeGeometry.faces[i];
    p = this.tubeGeometry.vertices[f.a];
    h = (Math.floor(Math.abs(noise.simplex3(p.x * 2, p.y * 2, p.z * 2 + delta)) * 55 * 290) * 0.01 + 180) / 360;
    rgb = hslToRgb(h, 0.6, 0.5);
